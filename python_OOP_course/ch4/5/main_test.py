...
def test(
    ...
    print("---------------------------------")
    print(f"Creating an archer named {
          archer_name} with {archer_arrows} arrows")
    archer = Archer(archer_name, archer_arrows)
    print(
        f"Creating a crossbowman named {
            crossbowman_name} with {crossbowman_bolts} bolts"
    )
        ...
        if expected_exception:
            print(
                f"Expected exception '{
                    expected_exception}', but no exception occurred"
            )
            print("Fail")
            return False
        elif crossbowman.get_num_arrows() != expected_remaining_bolts:
            print(
                f"Expected remaining arrows: {expected_remaining_bolts}, Actual: {
                    crossbowman.get_num_arrows()}"
            )
            print("Fail")
            return False
        else:
            print(
                f"Expected {expected_remaining_bolts} remaining bolts, Actual: {
                    crossbowman.get_num_arrows()}"
            )
            print("Pass")
            return True
