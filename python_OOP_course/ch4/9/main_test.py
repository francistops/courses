...
def test(wizard, archer, actions, expected_result, expected_err=None):
    ...
    try:
        for action in actions:
            if action == "cast":
                print(f"{wizard.get_name()} casts a spell at {
                      archer.get_name()}")
                wizard.cast(archer)
            elif action == "shoot":
                print(f"{archer.get_name()} shoots an arrow at {
                      wizard.get_name()}")
                archer.shoot(wizard)
        ...
        print(
            f"Expected: {wizard.get_name()} HP: {expected_result[0]}, {
                archer.get_name()} HP: {expected_result[1]}"
        )
        print(
            f"Actual: {wizard.get_name()} HP: {wizard_hp}, {
                archer.get_name()} HP: {archer_hp}"
        )
