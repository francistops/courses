...
def test(vehicle, distance, fuel_price, expected_cost, expected_cargo_volume):
    try:
        ...
        print("---------------------------------")
        print(
            f"Testing {vehicle_type}: Max Speed {
                vehicle.max_speed} kph, Efficiency {vehicle.efficiency} km/food"
        )
        print(f"Distance: {distance} km, Price: {fuel_price} per food")
        print(
            f"Expected: Trip Cost: {expected_cost}, Cargo Volume: {
                expected_cargo_volume}"
        )
        ...
        print(
            f"  Actual: Trip Cost: {actual_cost}, Cargo Volume: {
                actual_cargo_volume}"
        )
        if (
