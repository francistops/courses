...
def test(input1, input2, expected_output):
    ...
    try:
        ...
        if actualSoliderOneDps != expectedSoliderOneDps:
            print(
                f"get_soldier_dps({input1}) returned {actualSoliderOneDps}, but expected {
                    expectedSoliderOneDps}"
            )
            return False
        if actualSoliderTwoDps != expectedSoliderTwoDps:
            print(
                f"get_soldier_dps({input2}) returned {actualSoliderTwoDps}, but expected {
                    expectedSoliderTwoDps}"
            )
            return False
        print("Pass")
        return True
